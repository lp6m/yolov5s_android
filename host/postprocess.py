...
def plot_and_save(bboxres, input_img, path):
    ...
    for i, det in enumerate(bboxres):  # detections per image
        ...
        if len(det):
            ...
            # det[:, :4] = scale_coords(input_img.shape[1:], det[:, :4], input_img.shape).round()
            # det[:, :4] = scale_coords(input_img.shape[2:], det[:, :4], input_img.shape).round()
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                label = names[c].strip()
                print(label, xyxy)
                plot_one_box(xyxy, input_img, label=label, color=colors(c, True), line_thickness=2)
